    ...
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse" id="navbar-ex-3">
        <div v-if="client.ships.length" class="navbar-nav me-auto">
          ...
          <a class="nav-item nav-link" v-for="ship in client.ships" :key="ship.id" :href="`/holds/ship/${ship.id}`"
            @click="handleNavigation(ship)">{{ ship.name }}</a>
        </div>
        <div class="btn-group mx-1">
          ...
          <ul class="dropdown-menu">
            <li><a @click.prevent="changeLanguage('fr')" class="dropdown-item" href="javascript:void(0);">french</a>
            </li>
            <li><a @click.prevent="changeLanguage('en')" class="dropdown-item" href="javascript:void(0);">english</a>
            </li>
          </ul>
        </div>
